...
INSERT INTO saldos_tercero   
			SELECT distinct coalesce(sum(res.suma_credito),0) as suma_credito, coalesce(SUM(res.suma_debito),0) as suma_debito,  
						coalesce(res.tercero,'.') as tercero, res.cuenta as cuenta,  2020 as anio , 2 as mes,   
			...
			FROM  ((SELECT distinct coalesce(sum(m.credito),0) as suma_credito, coalesce(SUM(m.debito),0) as suma_debito,    
								coalesce(m.cod_tercero,'.') as tercero, c.cod_cuenta as cuenta,  2020 as anio , 2 as mes, 0 as saldo_credito, 0 as saldo_debito   
					...
					union  
					(SELECT distinct 0 as suma_credito, 0 as suma_debito, coalesce(st.cod_tercero,'.') as tercero, st.cod_cuenta as cuenta,   
								2020 as anio , 2 as mes, coalesce(sum(st.saldo_credito),0) as saldo_credito, coalesce(sum(st.saldo_debito),0) as saldo_debito   
					FROM  saldos_tercero st where st.anio = 2020 and mes = 1   
